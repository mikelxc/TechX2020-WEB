...
const countriesData = require('./data.json');
const INum = JSON.parse(countriesData)
for (let NINum in INum){
...
function sortby (a,b){
    return confirmAdd.b-confirmAdd.a;
};
...
// TODO: Please write your code below
const countriesData = require('./data.json');
let rate = "dead"/"confirm";
countriesData.deathRate = rate
function sortby (a,b){
    return deathRate.b-deathRate.a;
};
countriesData.sort(sortby)
...
// TODO: Please write your code below
const countriesData = require('./data.json');
for (let dataAmerica in countriesData){
    document.getElementById("北美洲").value
};
...
const dataAmerica = data.filter(item => item.isUpdated=true)
...
const countryN= countriesData.map(country);
