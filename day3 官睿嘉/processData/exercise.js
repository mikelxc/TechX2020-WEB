...
// JS Object to JSON
const jsObj = JSON.parse(jsonStr)     //jsObj
// JSON string to JS Object
const str = JSON.stringify(jsObj)    //json str
...
// 想象你需要做一个统计新冠肺炎全球（除中国以外）感染数据的网站。为此，你从腾讯的API获取了感染数据的JSON。
//现在你需要对这些数据进行处理，以便使用。
...
// TODO: Please write your code below
    ...
    for(let country of countriesData){
        if(country.heal>theMost)
        {
        theMost=country.heal;
        theCou=country.name;
        }
    }
    console.log(theCou);
...
// TODO: Please write your code below
            ...
            countriesData.sort(compare('comfirmAdd'));
            countriesData.sort((a, b) => b.confirmAdd - a.confirmAdd)
...
// TODO: Please write your code below
            ...
            console.log(countriesData.sort(compare('deathRate')));
            /*法二:
            var tamp;
            for(var i=0;i<countriesData.length-1;i++) {
                for(var j=1;j<countriesData.length;j++){
                    if(countriesData[i].deathRate>countriesData[j].deathRate){
                        tamp=countriesData[i];
                        countriesData[i]=countriesData[j];
                        countriesData[j]=tamp;
                }
            }
            }
            console.log(countriesData);
...
// 习题：要求写一个函数getSortBy。实现在一个json数组data内（例如countriesData，根据某种原则 valuefunc(object)，
// 给所有对象添加一个值(key)，并返回以降序排序用的sortby函数。
// 返回值是一个sortby函数
// 参数：data, key,  function valuefunc(object)
// 然后用getSortBy生成一个"healingRate"字段(heal/confirm)，并根据这个字段降序排序。
// TODO: Please write your code below
            function getSortBy(data, key, valuefunc){
                for(let i=0;i<data.length;i++) {
                    data[i].key=valuefunc(countriesData[i]);
                }  
                return compare(key);
            }
            countriesData.sort(getSortBy(countriesData,'healingRate',function valuefunc(object){}));
...
// 请用这种方式实现同样的功能
// TODO: Please write your code below
        let  dataAmerica2 = countriesData.filter(items => items.continent=="北美洲");
...
// TODO: Please write your code below
            const nameOfC = (item) => (item.name);
            const nameList=countriesData.map(nameOfC);
