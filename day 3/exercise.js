...
console.log(countriesData[index].name);
...
//   console.log(i);
//   for (let key in countriesData[i]) {
//     console.log(key + " is " + countriesData[i][key]);
//   }
// }

// 习题：请在countriesData中为各个国家添加死亡比例deathRate字段(累计死亡人数/累计确诊人数人数)并进行降序排序
function getSortBy(a, b) {
  a.healingRate = a.heal / a.confirm;
  b.healingRate = b.heal / b.confirm;
  return -a.healingRate + b.healingRate;
}
...
let dataAmerica = countriesData.filter((item) => item.continent == "北美洲");
...
console.log(countriesData.map((item) => item.name));
