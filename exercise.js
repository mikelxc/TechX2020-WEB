...
countriesData = require("./data.json");
// 下面是习题部分
// // 1- 请找出治愈人数最多的国家
...
for (element in countriesData) {
  CureArr[element] = countriesData[element].heal;
}
console.log(countriesData[CureArr.indexOf(Math.max.apply(null, CureArr))].name);
...
for (element in countriesData) {
    console.log (countriesData[element].name + "  confirmAdd: " + countriesData[element].confirmAdd);
}
...
for (element in countriesData) {
  countriesData[element].deathRate = countriesData[element].dead /countriesData[element].confirm;
}

countriesData.sort((a,b)=>b.deathRate-a.deathRate);

for (element in countriesData) {
    console.log (countriesData[element].name + "  deathRate: " + countriesData[element].deathRate);
}
...
getHealingRate = (object) => object.healingRate = object.heal /object.confirm;

countriesData.sort(getSortBy(countriesData,"healingRate",getHealingRate));
...
dataAmerica = countriesData.filter((item) => item.continent == "北美洲")
...
const getname = (num) => num.name
const NameList = countriesData.map(getname);
console.log(NameList)
//require('/mnt/d/CS/exercise.js')