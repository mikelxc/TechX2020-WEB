...
let healmax=countriesData[0];
for(let i in countriesData){
if(countriesData[i].heal>healmax.heal){
    healmax=countriesData[i];
}
}
console.log(healmax.name+"的治愈人数最多")
...
let arrAdd=[];
for(let i in countriesData){
    arrAdd.push(countriesData[i].confirmAdd);
}
...
for(let i in countriesData){
    let DR=countriesData[i].dead/(countriesData[i].confirm);
    countriesData[i].deathRate=DR;
    arrDR.push(DR);
}
arrDR.sort(sortby);
//console.log(arrDR);
...
function getSortBy(data, key, valuefunc){
    for(let i in data){
        countriesData[i][key]=valuefunc(countriesData[i]);
    }
    return (a,b)=>b[key]-a[key];
}

let arrHealingRate=countriesData.sort(getSortBy(countriesData,"healingRate",(object)=>object.heal/object.confirm))

console.log(arrHealingRate);
// 4- 我们只需要北美洲的信息
...
// 请用这种方式实现同样的功能
// TODO: Please write your code below
let dataAmerica2=countriesData.filter(item=>item.continent=="北美洲")
...
let generateName=(item)=>item.name;
let namelist=countriesData.map(generateName);
