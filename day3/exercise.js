...
// 1- 请找出治愈人数最多的国家
function max(countriesData) {
  let max = countriesData[0].heal;
  let name = countriesData[0].name;
  for (var i = 0; i < countriesData.length; i++) {
    if (max < countriesData[i].heal) {
...
function addSort() {
  ...
  for (var i = 0; i < countriesData.length; i++) {
    result[i] = countriesData[i].confirmAdd;
  }
  result.sort(sort2);
  return result;
}
...
function deathRateSort(){
    let rateList = [];
    function sort2(a, b) {
        return b - a;
    }
    for (var i = 0; i < countriesData.length; i++) {
        rateList[i] = countriesData[i].dead / countriesData[i].confirm;
    }
    rateList.sort(sort2);
    return rateList;
}
console.log(deathRateSort());
...
sortbyHR =  getSortBy(countriesData, "healingRate", (a) => (a.heal / a.confirm));
countriesData.sort(sortbyHR);
...
// 请用这种方式实现同样的功能
const America = countriesData.filter((country) => country.continent == "北美洲");
console.log(America);
...
const name1 = (item) => item.name;
const nameLists = countriesData.map(name1);
