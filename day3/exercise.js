...
for (let i=1;i<countriesData.length;i++) {
    let c = countriesData[0].name;
    if (countriesData[i].heal > countriesData[i-1].heal) {
        c = countriesData[i];
    }
    return c;
}
...
for (let i = 0; i < countriesData.length ; i++){
    arr[i] = countriesData[i].confirmAdd;
}
arr.sort(sortby2);
...
for (let i = 0; i < countriesData.length; i++){
    ...
    array[i] = countriesData[i].deathRate;
}
array.sort(sortby2);
...
// 请用这种方式实现同样的功能
// TODO: Please write your code below
const NorthAmerica = countriesData.filter(item => item.continent == "北美洲");
...
const list = countriesData.map(name);
console.log(list);