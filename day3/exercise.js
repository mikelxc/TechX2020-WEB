...
let max = countriesData[23];
for(let i=0 ; i<countriesData.length; i++){
    if(countriesData[i].heal > max.heal)
        max = countriesData[i];
}
console.log("The country with the highest number of cured people is " + max.name);
// 2- 排序
...
function getSortBy(data, key, valuefunc){
    for(let element of countriesData){
        element[key] = valuefunc(element);
    }
    return (a,b) => b[key] - a[key];
}
sortbyHealingRate =  getSortBy(countriesData, "healingRate", (a) => (a.heal/a.confirm));
...
const name = countriesData.map(countryName);
console.log(name);
