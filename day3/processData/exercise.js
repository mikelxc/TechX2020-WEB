...
function findMaxHeal(){
    let maxHeal = 0;
    let currHeal = 0;
    let maxCountry = null;
    for (let i = 0; i <= 181; i++){
        currHeal = countriesData[i].heal;
        if(maxHeal > currHeal){
            maxHeal = currHeal;
            maxCountry = i;
        }
    return countriesData[maxCountry].name;
    }
}
...
countriesData.confirmAdd.sort(sortby);


// 习题：请在countriesData中为各个国家添加死亡比例deathRate字段(累计死亡人数/累计确诊人数)并进行降序排序

// TODO: Please write your code below
countriesData.deathRate = countriesData.dead/countriesData.nowConfirm
function sortby(a, b){
    return b.deathRate - a.deathRate;
}
countriesData.deathRate.sort(sortby);
...
for (country of countriesData){
    if(country.continent = "北美洲"){
        dataAmerica.push(country);
    }
}
...
// 请用这种方式实现同样的功能
// TODO: Please write your code below
const dataAmerica = countriesData.filter(item.continent = "北美洲")；
console.log(dataAmerica);
...
// TODO: Please write your code below
const getName = (countriesData) => (countriesData.name);
const countriesName = countriesData.map(getName);
